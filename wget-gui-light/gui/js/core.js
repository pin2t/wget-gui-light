...
$(function() {
  ...
  // Some global and important constants & functions
  var _LANG = (window.navigator.userLanguage || window.navigator.language).substring(0, 2).toLowerCase() || 'en',
      // Get language var, stored in "var l10n = {'ru': 'example': 'Example text'}}"
      __ = function(val, def){
  ...
  /* *** Localizate interface ****************************************************** */
  
  $('#urlbar > div.title').text(__('download_file', 'Download file')+':');
  $addTaskBtn
    .val(__('add_task', 'Add Task'));
  $('#historyList > h2')
    .text(__('recent_tasks', 'Recent tasks')+':');
  $('#taskExtended > .multitask')
    .html(__('press', 'Press')+' <span class="b">Ctrl</span> + <span class="b">Enter</span> '+__('add_more_task', 'if you want add more than one task'));
  $menuButton.find('span')
    .text(__('close', 'Сlose'));
  $('#bookmark')
    .text(__('bookmark_download', 'Bookmark "Download"'));
  $downloadedFilesList.find('h2')
    .text(__('downloaded_files', 'Downloaded files')+':');
  ...
  if(navigator.userAgent.toLowerCase().indexOf('firefox') > -1) {
    $addTaskAddr.height(17); // Fix Firefox auto-height bug
  }
  ...
  function getDownloadedFilesList(callback){
  ...
  var getFAiconByFileName = function(filename) {
      ...
      bytesToSize = function(bytes) { // Source from: <http://stackoverflow.com/a/18650828/2252921>
        var sizes = [__('bytes', 'Bytes'), 'KB', 'MB', 'GB', 'TB'];
        if (bytes == 0) return '0 ' + __('bytes', 'Byte');
        var i = parseInt(Math.floor(Math.log(bytes) / Math.log(1024)));
        return Math.round(bytes / Math.pow(1024, i), 2) + ' ' + sizes[i];
      },
      
      refreshDownloadedFilesList = function(){
        ...
        getDownloadedFilesList(function(filesListData){
          ...
          if((typeof filesListData == 'object') && filesListData) {
            ...
            if(guiFilesList.html() !== tempFilesList.html()) {
              if(DEBUG_MODE) console.log('Files list has been updated. Repaint');
  ...
  $addTaskAddr.on('keypress', function(e) {
    ...
    if (e.ctrlKey && (e.keyCode == 10 || e.keyCode == 13)) {
      if(taskFieldsLinesCount >= 7) { // maximum lines (tasks) count
        return false;
      }
  ...
  //showNoty({type: 'error', title: 'Title', msg: 'Message'});
  if(DEBUG_MODE) updateStatusInterval = 0;
  syncTasksList(); // Run timer // TODO: Uncomment
  //addTaskGui({'url': 'http://foo.com/bar.dat', 'progress': 23, 'id': 12345, 'name': 'James Blunt - (Live Text )- You are beautiful (My Life Is Brilliant) HD ) (720p).mp4'});
